...
```{r}
a=c(sum(stack$start),sum(stack$step2),sum(stack$step3),sum(stack$step4),sum(stack$step5),sum(stack$step6))
b=colnames(stack)[2:7]
temp=melt(stack,id="X__1")
p=ggplot(temp, aes(variable, value,group=X__1,color=X__1,fill=X__1))+geom_area(position = "stack")+theme(legend.position = "none")
ggplotly(p)
```
