...
senate_stats <- reactive({
  senatordata <- senatordata %>% 
      ...
      group_by(bioguideId) %>%
      summarise(Introduced = sum(n()), Passed = sum(billoutcome == "Became Law"), Percent = mean(billoutcome == "Became Law")*100)
  
  dat <- nodes %>% left_join(senatordata)
  
  # Sorting
  dat[order(dat$Introduced, decreasing = TRUE), ] %>%
    head(n = input$matrix_rows)
  })
...
renderHighchart({
  ...
  highchart() %>%
    ...
    hc_add_series(dat$Introduced, showInLegend = FALSE) %>%
    hc_add_theme(thm)
})
...
```{r}
#dat=melt(fittedprob,id=c("session","policy_area","party","state"))
...
renderPlot({ggplot(probdata(),aes(x=x_axis,y=y_axis,color=factor(party)))+
  geom_point(alpha=0.5,size=0.5)+facet_grid(session~.)+theme(axis.text.x = element_text(angle = 90, hjust = 1))+labs(y="Fitted Probability of Bills Become to Law")+scale_color_discrete(name="Senator's Party",position="bottom")})

#as.numeric(as.character(value))
#filter(grepl(input$methods, variable))
```

Row {data-height=400}
-------------------------------------
### Prediction
User can select a particular scenario, and comes out a predicted probability with 95% confidence interval.
Developing!
