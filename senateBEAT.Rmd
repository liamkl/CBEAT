...
```
```{r, eval = FALSE}
wordcloud2(data = word_matrix, shape="pentagon", size = 1)
```
...
senatorstats <- senatordata %>% 
    filter(grepl("113", session)) %>% 
    group_by(state, senator) %>% 
    filter(grepl("Alaska", state_name))
...
```{r, echo = FALSE}
network <- senatordata %>% 
    filter(grepl("113", as.character(session))) 
...
renderPlot({ggplot(probdata(),aes(x=x_axis,y=y_axis,color=factor(party)))+
  geom_point(alpha=0.5,size=1)+labs(y="Fitted Probability of Bills Become to Law",x="")+scale_color_discrete(name="Senator's Party",position="bottom")+theme(axis.text.x = element_text(angle = 45, hjust = 1))})
```
