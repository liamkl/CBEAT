...
In order to access our full GitHub data repository, please click [here](https://github.com/liamkl/CBEAT).
...
There are two major issues with our data-set when modeling the probability of bills get to certain stage. First, some of the categories have very few observation. Second, since many of our predictors are categorial variables (i.e., states, topics, interaction terms), we have too many parameters in the model. Regularization is a technique used to avoid this overfitting problem. The idea behind regularization is to penalize the models that overfit the data. Lasso uses L1 norm and performs both variable selection and regularization in order to enhance the prediction accuracy. Ridge regularization uses L2 norm and works well when the data suffers from multicollinearity. 
...
We looked into bills that are from the 113th, 114th, and 115th legislative sessions (Jan 2013 - Oct 2017).  

### 4.2 Data Scraping

We scraped the 115th session bills from https://www.gpo.gov/fdsys/bulkdata/BILLSTATUS/, using the code below. The data is saved under rawdata folder. In addition, we were able to download the 113 and 114 session bills in bulk from congress.gov, which saved us a few hours. 
...
#### 4.3.3 Senator Subject Interests Data

We extracted from each bill's XML file the legislative subjects associated with it, and created a matrix featuring each Senator as a row, and each column as a legislative subject. The values are then the number of bills sponsored by that Senator which pertains to that legislative subject.
...
#### 4.3.4 Networks Edges and Nodes Data
...
Even though we retrieved the bills from both the Senate and the House, we decided to focus on bills that are generated from the Senate. Specifically, we are exploring the 8769 bills that were created from the 113th to 115th sessions (to date) in the Senate.
...
Here we see how many bills get to each step of the process. Over 8000 bills were introduced to the Senate, however it is extremely hard for a bill to even get to the second stage, order reported (only about one-eighth of the bills get reported). Eventually, only 215 of the bills go on to become laws.
...
colnames(table)="Number of bills"
kable(table, align="c")
```
...
We can see the most frequent key words in bills from the Senate is, perhaps ironically, "Congressional Oversight":
...
### 5.4 Senator Sepcific Summary Statistics

While the homepage focuses on interesting senator activity findings, we explored different data visualizations to help users better understand state or person specific activitiy. We also included images of senators, to help users put a face to the name. 

#### 5.4.1 Senator summary statistics 
...
toppolicy <- senatordata %>% 
    filter(grepl("114", session)) %>% 
          group_by(state,senator,policy_area) %>% filter(grepl("Louisiana", state_name)) %>% 
...
#### 5.4.2 Senator images

For our Senator Activity page, we pull images in real time from Congress.gov, using the Senator's ID.

```{r}
#display first senator
#format to make data into url that displays
img_urls <- unique(senatorstats$image)
src <- img_urls[1]
tags$img(src=src)

#display second senator
#format to make data into url that displays
img_urls <- unique(senatorstats$image)
src <- img_urls[2]
tags$img(src=src)
...
#### 5.4.3 Senator Bill Submitted 
We see from the bar plot that Republician Senator David Vitter from Louisiana submitted the most number of bills, 204 bills. This visualization will be very useful when we break it down by session since some of the Senators stayed in the Senate longer the others. The interactive plot in our website provides the ability for the user to filter by session.

```{r}
senatordata1 <- mapdata
senatordata1$senator <- paste(senatordata1$first_name,senatordata1$last_name,sep=" ")
senatordata1 <- senatordata1 %>% 
      group_by(bioguideId) %>%
      summarise(Introduced = sum(n()), Passed = sum(billoutcome == "Became Law"))
  dat <- nodes %>% left_join(senatordata1)
...
#### 5.4.4 Senator Bill Passed 
Some Senators have submitted hundreds of bills, but how many of those actually passed? We can see in the plot below.
...
#manipuating data to get necessary totals
dat3 <- mapdata3 %>% group_by(session) %>% mutate(total_session = sum(n())) %>% ungroup()


#initial map created for exploration
dat3 <- dat3 %>% group_by(session, state) %>% mutate(total_submit = sum(n()), proportion = total_submit/total_session) %>% ungroup()

dat4 <- dat3 %>% filter(session == "113")

#mutating state names to full names;  need to add in for PR, MP,  DC, GU, AS, VI
dat4$state_name_2 <- tolower(dat4$state_name)
...
q <-  ggplot(fifty_states, aes(map_id = id)) + geom_map(map = fifty_states, fill="lightgrey", colour = alpha("grey", 3/4), size = 0.4)+ geom_map(aes(map_id = state_name_2, fill=total_submit), colour=alpha("darkgrey", 3/4), data=dat4, map=fifty_states) +
  expand_limits(x = fifty_states$long, y = fifty_states$lat) +
  coord_map() +
  scale_x_continuous(breaks = NULL) + 
  scale_y_continuous(breaks = NULL) +
...
##table summarizing the total of a state, just to explore what these stats could mean

dat5 <- mapdata

dat5$senator <- paste(dat5$first_name,dat5$last_name,sep=" ")

dat5 %>% group_by(state, senator) %>% filter(state_name=="Alaska") %>% summarize(Total = sum(n()))
...
While our data exploration and analysis was performed in Rmarkdown, our final website was published using Rshiny and Flexdashboard. These images provide an overview of the final visualizations we included in our website, but the interactive versions can be found on our website [link](https://yuetingluo.shinyapps.io/senateBEAT/).
...
For this analysis, we were interested in understanding which legislative subjects Senators were interested in, and finding out who had similar interests. 
...
### 6.4 Senator Activity 

#### 6.4.1 Website homepage highlights

Using this tool, users can explore specific state and senator activity. Our homepage focused on some highlights:
...
If you continue your exploration, take a look at the Louisiana during the 113th session. Louisiana sponsored the second most bills during the 113th, and yet this time, none of them passed. If I were a constituent from LA, I would be concerned about these numbers. 
...
### 6.5 - Bills Submitted by New Mexico's Senators
...
### 6.6 - Explore geographical differences in policy area interests with an interactive map
...
Above are some of the final analyses we made, yet overall the tool is meant to be utilized by the public. To explore our interactive website, please [click here](https://yuetingluo.shinyapps.io/senateBEAT/), and to view our video, please [watch here](https://youtu.be/D_irwj9-RSM). 

Access the original Rmarkdown for our website (in order to see how we created the dynamic versions of our images) [here](https://github.com/liamkl/CBEAT/blob/master/senateBEAT.Rmd).

**Note that the website performs best in Safari**