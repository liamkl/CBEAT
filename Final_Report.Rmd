...
Even though we retrieved the bills from both the Senate and the House, we decided to focus on bills that are generated from the Senate. Specifically, we are exploring the 8769 bills that were created from 113 to 115 session (to date) in the Senate.
...
```{r}
#we scraped data from the internet in order to obtain a unique url for a picture of each senator
...
We see from the bar plot that republician senator David Vitter from Louisina submitted the most number of bills, 204 bills. This visualization will be very useful when we break it down by session since some of the senator stayed in the Senate longer the others. The interactive plot in our website provides user to filter by sessions.
...
Some senators have submitted hundreds of bills, but how many are there that actually passed. We see from the plot below.
